...
function getChildrenLogs($index, $date)
{
  ...
  if ($index == 0) { //no specific child 
    if (!empty($date)) { //and specific date
      $sqlString = "SELECT D.*,C.fname,C.lname,C.`user-id` FROM `dailylog` D INNER JOIN CHILD C ON D.`child-id`=C.id WHERE C.`user-id` = ? && D.date = ? ORDER BY D.date DESC;";
      ...
      $sqlString = "SELECT D.*,C.fname,C.lname,C.`user-id` FROM `dailylog` D INNER JOIN CHILD C ON D.`child-id`=C.id WHERE C.`user-id` = ? ORDER BY D.date DESC;";
      $statement = $GLOBALS['db']->prepare($sqlString); //preapre the above statement
    ...
    if (!empty($date)) { //and specific date
      $sqlString = "SELECT D.*,C.fname,C.lname,C.`user-id` FROM `dailylog` D INNER JOIN CHILD C ON D.`child-id`=C.id WHERE C.`user-id` = ? AND D.`child-id` = ? AND D.date= ? ORDER BY D.date DESC;";
      $statement = $GLOBALS['db']->prepare($sqlString); //preapre the above statement
      ...
      $sqlString = "SELECT D.*,C.fname,C.lname,C.`user-id` FROM `dailylog` D INNER JOIN CHILD C ON D.`child-id`=C.id WHERE C.`user-id` = ? AND D.`child-id` = ? ORDER BY D.date DESC;";
      $statement = $GLOBALS['db']->prepare($sqlString); //preapre the above statement
